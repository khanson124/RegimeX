/**
 * XAU MTF Structure Momentum v1 research (no enablement / no existing strategy mutation).
 *
 *   pnpm --filter @regimex/worker exec vitest run src/cfd/xauMtfStructureMomentumResearch.test.ts
 */
...
describe("xau-mtf-structure-momentum-v1 research", () => {
  it(
    ...
    async () => {
      ...
      // Phase 1–2: development sensitivity (holdout untouched)
      const sensitivity = await runOneAtATimeSensitivity(
        chrono.developmentWeeks,
        observedSpreadBps
      );
      const sensitivityCollapse = detectSensitivityCollapse(sensitivity);

      // Phase 3–4: weekly matrix + chronological holdout for both execution modes
      const weekly5m = await runWeeklyXauMtfMatrix(usable, params5m, profiles, "xau-mtf-5m-entry");
      const weekly1m = await runWeeklyXauMtfMatrix(usable, params1m, profiles, "xau-mtf-1m-entry");

      const pooled5mObs = aggregateWeeklyResults(weekly5m, "xau-mtf-5m-entry", "OBSERVED_SPREAD_ONLY");
      const pooled5mZero = aggregateWeeklyResults(weekly5m, "xau-mtf-5m-entry", "ZERO");
      const pooled1mObs = aggregateWeeklyResults(weekly1m, "xau-mtf-1m-entry", "OBSERVED_SPREAD_ONLY");

      const [dev5m, hold5m, hold5mSlip, hold1m] = await Promise.all([
        runWeeksPooledXauMtf(chrono.developmentWeeks, params5m, observedSpreadBps, 0),
        runWeeksPooledXauMtf(chrono.holdoutWeeks, params5m, observedSpreadBps, 0),
        runWeeksPooledXauMtf(chrono.holdoutWeeks, params5m, observedSpreadBps, 0.25),
        runWeeksPooledXauMtf(chrono.holdoutWeeks, params1m, observedSpreadBps, 0)
      ]);
      ...
      const dominance = weekDominanceShare(weekly5m, "xau-mtf-5m-entry", "OBSERVED_SPREAD_ONLY");
      const slip025 = aggregateWeeklyResults(weekly5m, "xau-mtf-5m-entry", "ASSUMED_SLIP_0_25");

      const classification = classifyXauMtfResearch({
      ...
      // Baselines on same holdout weeks (observed spread) — do not overwrite prior artifacts
      const baselineHoldout: Record<string, unknown> = {};
      for (const id of ["squeeze-breakout-v1", "breakout-momentum-v1", "ema-pullback-v1"] as const) {
        const allTrades = [];
        for (const w of chrono.holdoutWeeks) {
          const r = await runBaselineOnCandles(w.candles, id, observedSpreadBps, 0);
      ...
      const costSensitivity = profiles.map((p) => {
        const a = aggregateWeeklyResults(weekly5m, "xau-mtf-5m-entry", p.id);
        return {
          profileId: p.id,
          label: p.label,
      ...
      const artifact = {
        ...
        defaults: XAU_MTF_STRUCTURE_MOMENTUM_DEFAULTS,
        ...
        structureStates: structureStateBreakdown(
          hold5m.trades.length ? hold5m.trades : dev5m.trades
        ),
        costSensitivity,
      ...
      const outPath = resolve(outDir, "XAUUSD_mtf_structure_momentum_research.json");
