/**
 * CFD stop/target for xau-volatility-expansion-retest-v1.
 * Invalidation stop from retest extreme; target R capped by structural room.
 */
import {
  type Candle,
...
export interface XauVolatilityExpansionRetestCfdParams {
...
export const DEFAULT_XAU_VOLATILITY_EXPANSION_RETEST_CFD_PARAMS: XauVolatilityExpansionRetestCfdParams =
  {
    targetRMultiple: 2,
    stopAtrMultiple: 1.5,
...
export function proposeXauVolatilityExpansionRetestStopTarget(input: {
  direction: PositionDirection;
  entryPrice: number;
  features: MarketFeatureSnapshot;
  candles: ReadonlyArray<Candle>;
  params?: Partial<XauVolatilityExpansionRetestCfdParams>;
  metadata?: Record<string, unknown>;
}): StopTargetProposal | null {
  const p = mergeCfdParams(DEFAULT_XAU_VOLATILITY_EXPANSION_RETEST_CFD_PARAMS, input.params);
  ...
  if (direction === "BUY") {
    if (swingLow != null && swingLow < entryPrice) {
      ...
      reasons.push(`Invalidation stop below retest low ${swingLow}`);
    ...
    reasons.push(`Invalidation stop above retest high ${swingHigh}`);
  } else if (atr != null && atr > 0) {
    stopLoss = entryPrice + atr * p.stopAtrMultiple;
    stopMethod = "atr_fallback";
    reasons.push(`ATR fallback stop ${p.stopAtrMultiple}×ATR`);
