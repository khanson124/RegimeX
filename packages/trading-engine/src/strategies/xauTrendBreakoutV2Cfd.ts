/**
 * CFD stop/target for xau-trend-breakout-v2 (research CFD backtests).
 */
import {
  type Candle,
...
export interface XauTrendBreakoutV2CfdParams {
...
export const DEFAULT_XAU_TREND_BREAKOUT_V2_CFD_PARAMS: XauTrendBreakoutV2CfdParams = {
  targetRMultiple: 2,
  stopAtrMultiple: 1.5,
...
export function proposeXauTrendBreakoutV2StopTarget(input: {
  direction: PositionDirection;
  entryPrice: number;
  features: MarketFeatureSnapshot;
  candles: ReadonlyArray<Candle>;
  params?: Partial<XauTrendBreakoutV2CfdParams>;
  metadata?: Record<string, unknown>;
}): StopTargetProposal | null {
  const p = mergeCfdParams(DEFAULT_XAU_TREND_BREAKOUT_V2_CFD_PARAMS, input.params);
  ...
  const consolLow =
    typeof input.metadata?.consolidationLow === "number"
      ? input.metadata.consolidationLow
      : typeof input.metadata?.pullbackLow === "number"
        ? input.metadata.pullbackLow
        : null;
  const consolHigh =
    typeof input.metadata?.consolidationHigh === "number"
      ? input.metadata.consolidationHigh
      : typeof input.metadata?.pullbackHigh === "number"
        ? input.metadata.pullbackHigh
        : null;
  ...
  if (stopLoss == null && atr != null && atr > 0) {
    ...
    if (input.direction === "BUY") {
      const struct =
        consolLow != null && consolLow < input.entryPrice ? consolLow - buffer : null;
      stopLoss = struct != null ? Math.min(atrStop, struct) : atrStop;
      stopMethod = struct != null ? "atr_structure_conservative" : "atr";
    } else {
      const struct =
        consolHigh != null && consolHigh > input.entryPrice ? consolHigh + buffer : null;
      stopLoss = struct != null ? Math.max(atrStop, struct) : atrStop;
      stopMethod = struct != null ? "atr_structure_conservative" : "atr";
    }
    reasons.push("ATR/structure consolidation stop");
  }
